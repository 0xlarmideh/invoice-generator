...
function jsPDFInvoiceTemplate(props) {
  ...
  param.invoice.table.forEach(function (row, index) {
    ...
    //body borders
    if (param.invoice.tableBodyBorder) addTableBodyBorder(maxHeight + 4);
    ...
    row.forEach(function (rr, index) {
      ...
      if (index == 0) doc.text(item.text, 11, currentHeight + 6);
      else {
        ...
        const widthToUse =
          currentTdWidth == previousTdWidth ? currentTdWidth : previousTdWidth;
        startWidth += widthToUse;
        doc.text(item.text, 11 + startWidth, currentHeight + 6);
      }
    });
